...
* Evaluate the performance of these technologies, either using models contained within the library, through the similar method exposed on each, i.e., a solar panel should be callable using the same methods as a heat pump (i.e., both should inherit the same structure for interacting with environmental and energistic inputs and outputs) such that its performance can be evaluated.
